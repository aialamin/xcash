      ...
      <View className="flex-row gap-2 p-4">
        {tabs.map(({ id, Icon, label }) => (
          <TouchableOpacity key={id} onPress={() => setTab(id as any)}
            className={`flex-1 flex-row items-center justify-center gap-1.5 py-2 rounded-xl border-2 ${tab === id ? 'border-violet-500 bg-violet-50' : 'border-gray-100 bg-white'}`}>
            <Icon size={15} color={tab === id ? '#7C3AED' : '#9CA3AF'} strokeWidth={1.8} />
            <Text className={`text-sm font-semibold ${tab === id ? 'text-violet-700' : 'text-gray-500'}`}>{label}</Text>
          </TouchableOpacity>
        ))}
      </View>
      <ScrollView className="flex-1 px-4" contentContainerClassName="gap-4 pb-8">
          ...
          <>
            {savings.map(s => (
              <View key={s.id} className="bg-white rounded-2xl p-4">
                ...
                <View className="w-full bg-gray-100 rounded-full h-2 mb-1">
                  <View className="bg-violet-500 h-2 rounded-full" style={{ width: `${Math.min(100, (s.current_amount / (s.target_amount || 1)) * 100)}%` }} />
                </View>
            ...
            {done !== 'savings' ? (
              <View className="bg-white rounded-2xl p-4 gap-3">
                <View className="flex-row items-center gap-2"><TrendingUp size={16} color="#7C3AED" /><Text className="font-semibold text-gray-700">Open New Savings</Text></View>
                {['general','dps','goal'].map(t=>(
                  <TouchableOpacity key={t} onPress={()=>setForm({...form,type:t})} className={`py-2 px-4 rounded-xl border-2 ${form.type===t?'border-violet-500 bg-violet-50':'border-gray-100'}`}>
                    <Text className={`text-sm font-semibold capitalize ${form.type===t?'text-violet-700':'text-gray-500'}`}>{t==='dps'?'DPS (Monthly)':t==='goal'?'Goal-based':'General'}</Text>
                  </TouchableOpacity>
                ))}
        ...
        {tab === 'loan' && (
          <>
            ...
            {done !== 'loan' ? (
              <View className="bg-white rounded-2xl p-4 gap-3">
                <View className="flex-row items-center gap-2"><CreditCard size={16} color="#7C3AED"/><Text className="font-semibold text-gray-700">Apply for Loan</Text></View>
                ...
                <View className="flex-row gap-2">{[3,6,12,18,24].map(t=>(<TouchableOpacity key={t} onPress={()=>setForm({...form,tenure:String(t)})} className={`flex-1 py-2 rounded-xl border-2 items-center ${form.tenure===String(t)?'border-violet-500 bg-violet-50':'border-gray-100'}`}><Text className={`text-xs font-semibold ${form.tenure===String(t)?'text-violet-700':'text-gray-500'}`}>{t}m</Text></TouchableOpacity>))}</View>
                {form.loanAmt?<Text className="text-xs text-violet-600 text-center bg-violet-50 rounded-xl py-2">Est. EMI: ৳{Math.ceil((Number(form.loanAmt)*1.12)/Number(form.tenure))}/month</Text>:null}
                <TouchableOpacity onPress={async()=>{try{const l=await api.post('/financial/loan',{amount:Number(form.loanAmt),tenure:Number(form.tenure)});setLoans(p=>[...p,l.data]);setDone('loan');}catch{}}} className="bg-violet-600 rounded-2xl py-3 items-center"><Text className="text-white font-bold">Apply Now</Text></TouchableOpacity>
              </View>
        ...
        {tab === 'insurance' && (
          <View className="bg-white rounded-2xl p-8 items-center gap-4">
            ...
            <TouchableOpacity className="bg-violet-50 rounded-2xl py-3 px-6"><Text className="text-violet-600 font-semibold">Notify Me</Text></TouchableOpacity>
          </View>
        )}
      </ScrollView>
    </View>
