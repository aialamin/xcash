      ...
      <ScrollView className="flex-1 p-4" contentContainerClassName="gap-4">
        {step === 'form' && (
          <>
            <View className="bg-white rounded-2xl p-4 gap-3">
              ...
              {Number(amount) > 0 && (
                <View className="bg-violet-50 rounded-xl p-3 gap-1">
                  ...
                  <View className="flex-row justify-between border-t border-violet-100 pt-1"><Text className="font-bold text-violet-700 text-sm">Total</Text><Text className="font-bold text-violet-700 text-sm">৳{Number(amount) + fee}</Text></View>
                </View>
              )}
            </View>
            ...
            <TouchableOpacity onPress={() => { if (agentPhone && amount) { setStep('pin'); setError(''); } }} className="bg-violet-600 rounded-2xl py-3 items-center">
        ...
        {step === 'pin' && (
          <View className="bg-white rounded-2xl p-6 items-center gap-4">
            <Text className="text-3xl font-extrabold text-violet-700">৳{amount}</Text>
