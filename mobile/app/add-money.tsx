...
const sources = [
  ...
  { id: 'card', Icon: CreditCard, label: 'Debit/Credit Card', bg: '#F0FDF4', color: '#166534' },
            ...
            <View className="bg-white rounded-2xl p-4 gap-3">
              ...
              {sources.map(({ id, Icon, label, bg, color }) => (
                <TouchableOpacity key={id} onPress={() => setSource(id)}
                  className={`flex-row items-center gap-3 p-3 rounded-xl border-2 ${source === id ? 'border-green-600' : 'border-gray-100'}`}
                  style={source === id ? { backgroundColor: '#F0FDF4' } : {}}>
                  <View className="w-9 h-9 rounded-xl items-center justify-center" style={{ backgroundColor: bg }}>
                    <Icon size={18} color={color} strokeWidth={1.8} />
                  </View>
                  <Text className={`font-medium flex-1 ${source === id ? 'text-green-800' : 'text-gray-700'}`}>{label}</Text>
                  {source === id && <Check size={16} color="#166534" />}
                </TouchableOpacity>
              ))}
            </View>
