      ...
      <ScrollView className="flex-1 p-4" contentContainerClassName="gap-4">
        {step === 'form' && (
          <>
            ...
            <TouchableOpacity onPress={()=>{if(form.merchantPhone&&form.amount){setStep('pin');setError('');}}} className="bg-violet-600 rounded-2xl py-3 items-center"><Text className="text-white font-bold text-lg">Proceed</Text></TouchableOpacity>
          </>
        )}
        {step==='pin'&&<View className="bg-white rounded-2xl p-6 items-center gap-4"><Text className="text-3xl font-extrabold text-violet-700">৳{form.amount}</Text><Text className="text-gray-500 text-sm">to {form.merchantPhone}</Text><PinPad onComplete={handlePin} label="Enter PIN to pay"/></View>}
        {step==='done'&&success&&(
