...
export default function Notifications() {
  ...
  return (
    <View className="flex-1 bg-green-50">
      <View className="px-4 pt-14 pb-3 bg-white border-b border-green-50 flex-row justify-between items-center">
        ...
        {notifs.some(n => !n.is_read) && (
          <TouchableOpacity onPress={markAll} className="flex-row items-center gap-1.5">
            <CheckCheck size={16} color="#166534" /><Text className="text-green-700 text-sm font-semibold">Mark all</Text>
          </TouchableOpacity>
        )}
      </View>
        ...
        {notifs.length === 0
          ...
          : notifs.map(n => (
            <TouchableOpacity key={n.id} onPress={() => markRead(n.id)}
              className={`bg-white rounded-2xl p-4 mb-2 flex-row gap-3 ${!n.is_read ? 'border-l-4 border-green-600' : ''}`}>
              <View className={`w-10 h-10 rounded-full items-center justify-center ${!n.is_read ? 'bg-green-100' : 'bg-gray-100'}`}>
                <Bell size={18} color={!n.is_read ? '#166534' : '#9CA3AF'} strokeWidth={1.8} />
              </View>
              ...
              {!n.is_read && <View className="w-2 h-2 bg-green-600 rounded-full mt-2" />}
            </TouchableOpacity>
          ))
        }
      </ScrollView>
