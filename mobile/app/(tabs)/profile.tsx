...
import { Users, Bell, Landmark, Shield, HelpCircle, FileText, ChevronRight, PlusCircle, LogOut, TrendingDown, MessageSquare, ShieldAlert, BarChart2, Gift } from 'lucide-react-native';
...
export default function Profile() {
  ...
  const menu = [
    ...
    { Icon: BarChart2,    label: 'Spending Analytics', to: '/analytics',     bg: '#F0FDF4', color: '#166534' },
    { Icon: Gift,         label: 'Refer & Earn',       to: '/referral',      bg: '#FFF7ED', color: '#F97316' },
    { Icon: Bell,         label: 'Notifications',      to: '/notifications', bg: '#F0FDF4', color: '#166534' },
    { Icon: Landmark,     label: 'Financial Services', to: '/financial',     bg: '#F0FDFA', color: '#0D9488' },
    { Icon: Shield,       label: 'Security Settings',  to: '/security',      bg: '#F0FDF4', color: '#16A34A' },
    { Icon: MessageSquare,label: 'Customer Support',   to: '/support',       bg: '#FFF7ED', color: '#F97316' },
    { Icon: FileText,     label: 'Terms & Privacy',    to: null,             bg: '#F9FAFB', color: '#6B7280' },
    ...(user?.role === 'admin' ? [{ Icon: ShieldAlert, label: 'Admin Panel', to: '/admin', bg: '#FEF2F2', color: '#EF4444' }] : []),
  ];
  ...
  return (
    <ScrollView className="flex-1 bg-green-50" showsVerticalScrollIndicator={false}>
      <View className="px-4 pt-14 pb-3 bg-white border-b border-green-50">
        <Text className="text-xl font-bold text-gray-800">My Profile</Text>
      </View>
      <View className="p-4 pb-24 gap-4">

        {/* Profile card */}
        <View className="rounded-2xl p-5 flex-row items-center gap-4" style={{ backgroundColor: '#166534' }}>
          ...
          <View className="flex-1">
            ...
            <Text className="text-green-200 text-sm">{user?.phone}</Text>
        ...
        <View className="bg-white rounded-2xl p-4 flex-row justify-between items-center">
          <View>
            ...
            <Text className="text-2xl font-extrabold text-green-800">৳{wallet?.balance?.toFixed(2) ?? '0.00'}</Text>
          </View>
          <TouchableOpacity onPress={() => router.push('/add-money')} className="flex-row items-center gap-1.5 rounded-xl px-4 py-2" style={{ backgroundColor: '#166534' }}>
        ...
        <View className="bg-white rounded-2xl p-4 gap-3">
          ...
          {dailySpend && (
            <>
              ...
              <View className="w-full bg-gray-100 rounded-full h-2">
                <View className={`h-2 rounded-full ${spentPct > 80 ? 'bg-red-500' : spentPct > 50 ? 'bg-amber-400' : 'bg-green-600'}`}
          ...
          {editLimit && (
            <View className="gap-2">
              ...
              <TouchableOpacity onPress={saveLimit} disabled={savingLimit} className="bg-green-700 rounded-xl py-2 items-center">
        ...
        <Text className="text-center text-xs text-gray-400">Pocket Enterprise v1.0 · Secure · Go + React Native</Text>
      </View>
    </ScrollView>
  );
}
