  ...
  return (
    <View className="flex-1 bg-green-50">
      <View className="flex-row items-center gap-3 px-4 pt-14 pb-4 bg-white border-b border-green-50">
        <TouchableOpacity onPress={() => router.back()} className="w-9 h-9 bg-green-50 rounded-full items-center justify-center">
      ...
      <ScrollView className="flex-1 p-4" contentContainerClassName="gap-4">
        {!done ? (
          <>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerClassName="gap-2">
                {operators.map(o => (
                  <TouchableOpacity key={o} onPress={() => setForm({ ...form, operator: o })}
                    className={`px-3 py-2 rounded-xl border ${form.operator === o ? 'border-green-600 bg-green-50' : 'border-gray-200'}`}>
                    <Text className={`text-sm font-medium ${form.operator === o ? 'text-green-800' : 'text-gray-600'}`}>{o}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              <View className="flex-row gap-2">
                {['prepaid', 'postpaid'].map(t => (
                  <TouchableOpacity key={t} onPress={() => setForm({ ...form, type: t })}
                    className={`flex-1 py-2 rounded-xl border-2 items-center ${form.type === t ? 'border-green-600 bg-green-50' : 'border-gray-100'}`}>
                    <Text className={`text-sm font-semibold capitalize ${form.type === t ? 'text-green-800' : 'text-gray-500'}`}>{t}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
              ...
              <View className="flex-row flex-wrap gap-2">
                {quickAmounts.map(p => (
                  <TouchableOpacity key={p} onPress={() => setForm({ ...form, amount: String(p) })}
                    className={`px-3 py-2 rounded-xl border-2 ${form.amount === String(p) ? 'border-green-600 bg-green-50' : 'border-gray-100'}`}>
                    <Text className={`text-sm font-semibold ${form.amount === String(p) ? 'text-green-800' : 'text-gray-600'}`}>৳{p}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            ...
            <TouchableOpacity onPress={submit} className="bg-green-700 rounded-2xl py-3 items-center">
          ...
          <View className="bg-white rounded-2xl p-8 items-center gap-4">
            ...
            <TouchableOpacity onPress={() => router.back()} className="bg-green-700 rounded-2xl py-3 px-8 w-full items-center">
